...
struct ClientInner {
    hosts: Hosts,
    open_conns: RwLock<HashMap<PublicKey, Arc<mux::Mux>>>,
...
impl ClientInner {
    ...
    fn existing_conn(&self, host: &PublicKey) -> Option<Arc<mux::Mux>> {
        let cache = self.open_conns.read().unwrap();
        cache.get(host).cloned()
    }

    async fn new_conn(&self, host: PublicKey) -> Result<mux::Mux, ConnectError> {
        let addresses = self
            .hosts
            .addresses(&host)
            .ok_or(ConnectError::UnknownHost(host))?;
        for addr in addresses {
            if addr.protocol != "siamux" {
                continue;
            }
